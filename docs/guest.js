...
const tasks={
 extract:{name:'Data extraction',owner:'Thom',role:'Data team'},
 ...
 synthesis:{name:'Results synthesis',owner:'Paul',role:'Assessment team'},
 report:{name:'Report',owner:'Kyuhan',role:'Author and reviewers'}
};
...
function updateJob(){
 ...
 $('job-page').hidden=!data.has_run||stage.status!=='completed';$('job-page').href='https://kyuhank.github.io/cpue-demo-'+taskFor(stage.key)+'/?job='+encodeURIComponent(stage.source_id);$('job-run').hidden=!stage.html_url;$('job-run').href=stage.html_url||data.run_url;
 $('job-outputs').disabled=!data.has_run||stage.status!=='completed';
