...
let pendingRun=null,liveRefreshing=false,lastLive=0;
...
function affected(){
 ...
 if(roots.includes('data')||(data.has_run===false&&!data.baseline_available))return new Set(['data',...data.stages.map(s=>s.key)]);
...
function sourceLink(key){
 ...
 link.title='Preview '+names[key];link.setAttribute('aria-label',link.title);
}
...
function node(key){
 ...
 el.append(control,link);el.onclick=()=>{selected=key;if(key==='data')dataDraft=String(data?.database_version||2023);hidePreview();draw();};
...
async function openCpueOutput(kind){
 ...
 hidePreview();$('source-full').hidden=true;$('source-full').removeAttribute('href');const ticket=++sourceTicket;$('source-title').textContent=names[stage.key];
...
function showPreview(key){
 ...
 $('preview-next').hidden=true;$('preview-full').hidden=!registered;$('preview-full').removeAttribute('href');if(registered)$('preview-full').href='https://github.com/'+source.repository+'/tree/'+source.commit;
 if(key==='data'){
...
function dataJobStatus(job,fallback='waiting'){return job?.skipped?'reused':job?.status||fallback;}
function setDataStatus(element,state){
 if(element.dataset.state===state)return;
 element.innerHTML=statusSymbol(state);
 element.append(document.createTextNode(state==='reused'?'Reused':states[state]||state));
 element.dataset.state=state;
}
const databaseActive=()=>data?.database_stage?.status==='running'&&(data.requested_roots?.length?data.requested_roots:runRoots||[selectedKey()]).some(k=>['data','extract'].includes(k));
function qualityStatus(){
 ...
 return q?(q.accepted?'completed':'failed'):'waiting';
}
function drawQuality(){
 let gate=$('quality-gate');if(!gate){
  gate=document.createElement('div');gate.id='quality-gate';gate.dataset.key='qc';gate.innerHTML='<button class="node-control"><strong>QC</strong><small></small></button>';
  gate.onclick=()=>{selected='data';dataDraft='new';hidePreview();draw();};
  $('chain').append(gate);
 }
 ...
 submission.dataset.status=phase==='resubmitting'?'running':dataJobStatus(actualSubmission);submission.classList.toggle('running',submission.dataset.status==='running');
 submission.querySelector('strong').textContent=phase==='resubmitting'?'Resubmit':'Data submission';
 let submissionState=submission.querySelector('.data-status');
 if(!submissionState){submissionState=document.createElement('small');submissionState.className='data-status';submission.querySelector('.node-control').append(submissionState);}
 setDataStatus(submissionState,submission.dataset.status);
 ingest.onclick=()=>{selected='data';dataDraft='new';hidePreview();draw();};
 let label=ingest.querySelector('.ingest-label');if(!label){label=document.createElement('button');label.type='button';label.className='ingest-label node-control';ingest.replaceChildren(label);}const state=qualityStatus();
 const ingestState=dataJobStatus(actualIngest,state==='failed'?'blocked':state==='completed'&&data.data_intake?.published?'completed':'waiting');
 if(!label.firstElementChild)label.innerHTML='<strong>Prepare & load</strong><small class="data-status"></small>';
 setDataStatus(label.querySelector('.data-status'),ingestState);
 ingest.className='ingest-node '+ingestState;submission.classList.toggle('needs-correction',state==='failed'&&phase!=='resubmitting');
 ...
 const gateState=phase==='resubmitting'?'waiting':data.intake_stages?.find(j=>j.key==='qc')?.skipped?'reused':state;
 gate.className='quality-gate '+gateState;setDataStatus(gate.querySelector('small'),gateState);
 gate.style.transform='translateY(-83px)';
 gate.title='View the data quality check';
 if(!ingest.querySelector('.source-link')){ingest.append(previewControl('ingest'));}sourceLink('ingest');
...
function links(){
 ...
 const all=[{key:'qc',parents:['submission'],status:qualityStatus()},{key:'ingest',parents:['qc'],status:stages.get('ingest')?.status},{key:'data',parents:['ingest'],status:data.database_stage?.status},...data.stages.map(s=>s.key==='extract'?{...s,parents:['data']}:s)];
...
function draw(){if(!data)return;const impact=affected(),key=selectedKey(),steps=(data.execution?.jobs||[]).flatMap(j=>j.steps||[]),active=steps.find(s=>s.status==='in_progress'),reused=data.stages.filter(s=>s.reused).length,busy=sending||!!expected||data.status!=='completed',stale=data.source?.stale,cloudBlocked=data.session&&!data.session.can_update;
 ...
 source.classList.toggle('running',databaseActive());
 source.classList.toggle('completed',data.database_stage?.status==='completed');
 source.classList.toggle('impacted',!!selected&&dataImpacted);
 source.classList.toggle('outside-impact',!!selected&&!dataImpacted);
 source.classList.toggle('reused',dataRetained&&!sending&&!expected);
 source.classList.toggle('selected',selected==='data');
 source.querySelector('.node-control').setAttribute('aria-pressed',String(selected==='data'));
 const intake=data.intake_stages||[],qc=data.data_intake?.quality_check;
 const awaitingLoad=intake.some(j=>j.key==='submission'&&!j.skipped)&&!intake.some(j=>j.key==='ingest'&&j.status==='completed');
 const version=(awaitingLoad?data.latest_database_version:data.database_version)||(data.data_intake?.published?qc?.proposed_version:null);
 source.querySelector('.release').textContent=version?'v'+version:'Snapshot';
 const databaseState=databaseActive()?'running':dataRetained?'reused':data.database_stage?.status||'waiting';
 setDataStatus(source.querySelector('.qc'),databaseState);
 for(const s of data.stages){const el=node(s.key),state=expected?(impact.has(s.key)||!selected?'waiting':s.status):s.status;el.className='stage '+state+(s.reused&&!expected?' reused':'')+(selected&&impact.has(s.key)?' impacted':'')+(selected&&!impact.has(s.key)?' outside-impact':'')+(selected===s.key?' selected':'');el.querySelector('.state-icon').innerHTML=statusSymbol(s.reused&&!expected?'reused':state);el.querySelector('.state-label').textContent=s.reused&&!expected?'Reused':states[state]||state;el.querySelector('.node-control').setAttribute('aria-pressed',String(selected===s.key));el.title=(s.parents.length?'Inputs: '+s.parents.map(p=>names[p]).join(' + '):'Input: accepted database release')+' · select to update downstream';}
 ...
 $('selection-title').textContent=key==='data'?'Data submission':names[key]+(key.startsWith('prepare_')||key.startsWith('assessment_')?' · CPUE '+(key.includes('vessel')?'A':'B'):'');
 $('progress').textContent=(data.has_run===false&&!data.baseline_available&&key!=='data'?'Prepare initial inputs → ':key==='data'?'Submit example → QC → ':pendingCount>1?pendingCount+' starting stages → ':'Run this stage → ')+count+' stages run'+(count<data.stages.length?' · '+(data.stages.length-count)+' kept':'');
...
const JOB_MESSAGES = {
  ...
  extract:['Extract the required records.','Run the saved SQL against the selected database release.'],
  cpue_vessel:['Estimate the CPUE index · analysis A.','Calculate a relative abundance index from the extracted records.'],
  cpue_year:['Estimate the CPUE index · analysis B.','Apply the alternative analysis to the same records.'],
  cpue_summary:['Compare the CPUE results.','Bring analyses A and B together in plots and tables.'],
  cpue_report:['Build the CPUE report.','Collect the CPUE comparisons and their recorded inputs.'],
  prepare_vessel:['Build the assessment inputs · CPUE A.','Combine the CPUE index and catch in the model’s required format.'],
...
function narrate(){
 ...
 let text;
 if(sending)text=['Requesting the run.','Save your selection and start the dependent jobs.'];
 else if(pendingRun){const seconds=Math.floor((Date.now()-pendingRun.started)/1000);text=pendingRun.run_id?['Waiting for a runner.','Request accepted · '+seconds+' s elapsed.']:['Confirming the request.','Saving the selected versions · '+seconds+' s elapsed.'];}
 else if(data.has_run&&data.status==='queued')text=['Waiting for a runner.','Run #'+data.number+' is accepted. Execution starts when a runner is available.'];
 ...
 if(transfers.length&&!intake&&!data.source?.stale){
  ...
  const target=document.createElement('strong');target.textContent=targets.join(' + ');line.append(caption,arrow,target);
...
function observeRun(next){
 ...
 if(data?.run_id===next.run_id&&data?.attempt===next.attempt){
  ...
  const phases=[null,'returned','resubmitting','rechecking','corrected'];
  next.intake_stages=(next.intake_stages||[]).map(value=>{
   const old=data.intake_stages?.find(s=>s.key===value.key);
   if(value.key==='qc'&&phases.indexOf(old?.correction_phase||null)>phases.indexOf(value.correction_phase||null))return old;
   return value.key==='qc'&&value.correction_phase?{...old,...value}:merge(old,value);
  });
  if(next.database_stage)next.database_stage=merge(data.database_stage,next.database_stage);
  if(data.status==='completed'&&next.status!=='completed'){next.status=data.status;next.conclusion=data.conclusion;}
 }
 data=next;return true;
}
async function refresh(force=false){
 if(window.workshopActive===false||refreshing||sending)return;
 const interval=pendingRun||sending||expected?750:data?.status!=='completed'?6000:data?.session&&!data.session.can_update?1000:10000;
...
async function openSource(key,recordedSource=null){
 hidePreview();$('source-full').hidden=true;$('source-full').removeAttribute('href');const ticket=++sourceTicket,body=$('source-body');$('source-title').textContent=key==='data'?'Database':'Code repository · '+stageTitle(key);$('source-meta').textContent='';body.replaceChildren();$('source-view').showModal();
 if(key==='data'){
 ...
 $('source-title').textContent=source.repository;$('source-full').hidden=false;$('source-full').href='https://github.com/'+source.repository+'/tree/'+source.commit;
