...
async function refresh(){try{
 ...
 if(loaded!==id){$('years').textContent=complete?'Loading…':'In progress';$('rows').textContent='This run’s report will appear when all jobs complete.';$('data').textContent=d.commit.slice(0,12);$('code').textContent='—';}
 if(complete&&loaded!==id){const response=await fetch('/api/output?job='+encodeURIComponent(id)+'&file=manifest.json');if(!response.ok)throw Error('Report artifact unavailable; use the completed example below the panel.');const m=await response.json();if(latest!==id)return;if(m.source_git_commit!==d.commit)throw Error('Input identity differs from the selected run.');
  $('years').textContent=m.first_year+'–'+m.last_year;$('rows').textContent=m.rows.toLocaleString()+' synthetic longline sets';$('data').textContent=m.source_git_commit.slice(0,12);$('data').href='https://github.com/'+m.source_repository+'/commit/'+m.source_git_commit;$('code').textContent=m.git_commit.slice(0,12);$('code').href='https://github.com/kyuhank/cpue-actions-demo/commit/'+m.git_commit;$('report').href='/api/output?job='+encodeURIComponent(id)+'&file=report.html';loaded=id;$('report').classList.remove('disabled');
 }
