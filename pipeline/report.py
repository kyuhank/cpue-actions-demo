...
if manifest.get('cpue_runs'):
    ...
    branch_record = ('<h2>03 · Prepare the assessment inputs</h2><p>Each CPUE series is checked against its recorded checksum, '
        ...
        '<h2>Four assessment cases, one comparison</h2><p>Each prepared input supplies two illustrative natural mortality assumptions: lower M = 0.20 and higher M = 0.30 per year. M is held constant across ages and years in each case. '
...
{branch_record}<h2>04 · Fit and compare the toy assessments</h2><table><tr><th>CPUE input</th><th>Fitted recruitment R₀</th><th>Fitted q</th><th>Fixed M (year⁻¹)</th><th>Log-index SSE</th></tr>{parameters}</table><p>A Schaefer model fits carrying capacity K and catchability q to each index and annual removals. Natural mortality M is fixed at 0.35 or 0.45 for each recorded case; initial B/K = 1 is fixed. This deterministic example has no process error or uncertainty propagation and is not the BET MFCL assessment.</p>
