...
manifest['execution'] = ('10 dependency-linked stages; changed stages execute and matching outputs are reused in one GitHub job' if manifest.get('execution_mode') == 'single_runner_steps' else 'independent GitHub jobs') if 'collection' in manifest else 'local stages using the same case definitions'
...
if manifest.get('workflow_plan'):
    ...
    update_record = f'<h2>This update: {rerun} stages executed · {10-rerun} reused</h2><p>Changed settings invalidate their dependent stages. Reused files retain their checksums and original run records. Trigger commit: <code>{html.escape(plan["trigger_commit"])}</code>.</p><table><tr><th>Stage</th><th>Output</th><th>Origin run</th><th>Stage settings</th></tr>{update_rows}</table>'
