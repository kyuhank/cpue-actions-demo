...
parallel_cpue = 'input_preparation' in manifest
manifest['stages'] = ['extract', 'cpue_vessel', 'cpue_year', 'assessment', 'report'] if parallel_cpue else ['extract', 'cpue', 'assessment', 'report']
manifest['dependencies'] = ({'extract': [], 'cpue_vessel': ['extract'], 'cpue_year': ['extract'],
    'assessment': ['cpue_vessel', 'cpue_year'], 'report': ['assessment']} if parallel_cpue else
    {'extract': [], 'cpue': ['extract'], 'assessment': ['cpue'], 'report': ['assessment']})
...
if parallel_cpue:
    ...
    branch_record = ('<h2>One extraction, two CPUE jobs</h2><p>The two CPUE jobs use the same extracted snapshot. '
        'Assessment waits for both outputs, checks their data/code/run identities and years, then assembles its inputs.</p>'
        '<table><tr><th>CPUE specification</th><th>GitHub job</th><th>Parent index SHA-256</th></tr>' + branch_rows + '</table>')
