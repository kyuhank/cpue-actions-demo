...
def plot(field, filename, label):
    ...
    for case in rows:
        ...
        dash = ' stroke-dasharray="8 6"' if field != 'observed_index' and case['setting'] == 'higher_r' else ''
    ...
    for i, case in enumerate(groups):
        label_text = labels[case['choice']] + (f" · r={float(case['r']):.2f}" if field != 'observed_index' else '')
        x, y = 65 + (i % 2)*330, 333 + (i//2)*26
        dash = ' stroke-dasharray="6 4"' if field != 'observed_index' and case['setting'] == 'higher_r' else ''
...
manifest['stages'] = ['extract', 'cpue_vessel', 'cpue_year', 'prepare_vessel', 'prepare_year', *[case['key'] for case in case_definitions], 'report']
...
values = ''.join('<tr><td>' + labels[x['choice']] + f'</td><td>{float(x["r"]):.2f}</td>' + ''.join(f'<td>{float(x[k]):.3f}</td>' for k in ('final_index', 'final_B_over_K', 'log_index_SSE')) + '</tr>' for x in rows)
...
if manifest.get('cpue_runs'):
    ...
    run_rows = ''.join(f'<tr><td>{labels[record["choice"]]}</td><td>{record["r"]:.2f}</td><td><code>{html.escape(record["job"])}</code></td><td><code>{html.escape(record["prepared_by"])}</code></td></tr>' for record in manifest['assessment_runs'].values())
    branch_record = ('<h2>03 · Prepare the assessment inputs</h2><p>Each CPUE series is checked against its recorded checksum, '
        ...
        'CPUE remains relative to its first year; catches are in tonnes; the toy has one area.</p>'
        '<table><tr><th>CPUE branch</th><th>Preparation run</th><th>Prepared input SHA-256</th></tr>' + prep_rows + '</table>'
        '<h2>Four assessment cases, one comparison</h2><p>Each prepared input supplies two growth settings: r = 0.35 and r = 0.45. '
        ...
        '<table><tr><th>CPUE input</th><th>Growth r</th><th>Assessment run</th><th>Prepared by</th></tr>' + run_rows + '</table>'
...
<h2>02 · Standardise CPUE and compare choices</h2><div class="plots">{plots}</div><table><tr><th>CPUE choice</th><th>Growth r</th><th>Latest CPUE / first year</th><th>Latest toy B/K</th><th>Log-index SSE</th></tr>{values}</table>
