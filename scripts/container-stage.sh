#!/bin/sh
set -eu
case "${1:-}" in extract|cpue|prepare_inputs|assessment|report) ;; *) echo 'Choose extract, cpue, prepare_inputs, assessment or report.' >&2; exit 2;; esac
...
docker run --rm --pull always --network none --user "$(id -u):$(id -g)" \
  ...
  --env TOY_DATA_REPOSITORY --env TOY_SOURCE_DATABASE --env TOY_CPUE_CHOICE --env TOY_ASSESSMENT_CASE --env GITHUB_JOB \
