import {change,changeable,current,consoleLines,output} from './github.ts';
...
async function status(){
 ...
 return {...s,database_connected:true,data_intake:intake,session:{can_update:can,remaining:limits.remaining,next_update:limits.next_update,message:!enabled()?'Cloud execution is awaiting the owner’s restricted GitHub connection.':pending?'The update is stored; waiting for GitHub to acknowledge the next run.':limits.remaining===0?'Daily limit reached. You can still inspect the current run.':!can?'Wait for the current run and the short update interval.':`${limits.remaining} shared updates available today.`}};
}
export async function handle(request:Request){
 ...
 try{
  ...
  const control=await rpc('workshop_state');if(control.last_request?.result?.previous_run===s.run_id&&control.last_request?.result?.published!==false)return reply({detail:'An earlier update is waiting for GitHub. Inspect its request before submitting another.'},409);
